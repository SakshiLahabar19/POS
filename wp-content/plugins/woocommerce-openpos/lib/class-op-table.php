...
if(!class_exists('OP_Table'))
{
    class OP_Table{
                ...
                while (false !== ($entry = readdir($handle))) {
                    ...
                    if ($entry != "." && $entry != ".." && strpos($entry,'takeaway') == 0) {
                        ...
                        if(strpos($entry,'.json') > 0)
                        {
                            ...
                            $file_removing_path = $this->generate_removing_file_path($entry);
        ...
        function generate_removing_file_path($file_name){
            $process_name = 'removing_';
            $process_name .= $file_name;
            return $this->_base_path.'/'.$process_name;
        }
        public function removeJsonTable($table_id,$force = false){
            ...
            if(file_exists($file_path))
            {
                ...
                $process_path = $this->generate_removing_file_path($process_name);
                    ...
                    while (false !== ($entry = readdir($handle))) {
                        ...
                        if ($entry != "." && $entry != "..") {
                            ...
                            if(strpos($entry,'.json') > 0)
                            {
                                $is_delete = false;
                                $file_path = $this->_bill_data_path.'/'.$entry;
                                $process_path = $this->generate_removing_file_path($entry);
