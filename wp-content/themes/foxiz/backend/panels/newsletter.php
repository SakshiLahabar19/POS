...
if ( ! function_exists( 'foxiz_register_options_newsletter' ) ) {
	...
	function foxiz_register_options_newsletter() {
		...
		return array(
			...
			'desc'   => esc_html__( 'Select settings for the popup newsletter.', 'foxiz' ),
			'icon'   => 'el el-envelope',
			'fields' => array(
				array(
					'id'    => 'newsletter_cover_info',
					'type'  => 'info',
					'title' => esc_html__( 'Please update cover images to your own images after importing the theme demo.', 'foxiz' ),
					'style' => 'warning'
				),
				array(
					'id'       => 'newsletter_popup',
				...
				array(
					...
					'subtitle' => esc_html__( 'This option use for "Scroll Distance" mode. Input a distance value (px) when visitor scrolling down to show the popup.', 'foxiz' ),
					'default'  => '2000'
				),
				array(
