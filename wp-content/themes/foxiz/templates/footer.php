...
if ( ! function_exists( 'foxiz_render_footer' ) ) :
	function foxiz_render_footer() {
		...
		$classes       = array( 'footer-wrap rb-section' );
...
if ( ! function_exists( 'foxiz_render_footer_widgets' ) ) {
	...
	function foxiz_render_footer_widgets( $layout ) {
		...
		$classes = array( 'footer-columns rb-columns is-gap-25 rb-container edge-padding' );
		switch ( $layout ) {
			case 5 :
				$classes[] = 'footer-5c';
				break;
	...
	function foxiz_get_footer_copyright() {
            ...
            <div class="bottom-footer-section">
				<?php if ( ! empty( $logo['url'] ) ) : ?>
                    <a class="footer-logo" href="<?php echo esc_url( home_url( '/' ) ); ?>" title="<?php bloginfo( 'name' ); ?>">
						<?php echo foxiz_get_logo_html( $logo );
						if ( foxiz_dark_mode() ) {
							if ( empty( $dark_logo['url'] ) ) {
								$dark_logo = $logo;
							}
							echo foxiz_get_logo_html( $dark_logo, false, 'logo-dark', 'dark' );
						} ?>
                    </a>
				<?php endif;
				if ( ! empty( $social ) ) : ?>
		...
		if ( ! empty( $copyright ) || ! empty( $menu ) ) : ?>
            <div class="copyright-inner">
				<?php
				if ( ! empty( $copyright ) ) {
					echo '<div class="copyright">' . wp_kses( wpautop( $copyright ), 'foxiz' ) . '</div>';
				}
				if ( ! empty( $menu ) ) {
					wp_nav_menu( array(
						'menu'        => $menu,
...
if ( ! function_exists( 'foxiz_footer_amp' ) ) {
	function foxiz_footer_amp() {
		...
		$classes    = array( 'footer-wrap amp-footer rb-section' );
		$background = foxiz_get_option( 'footer_background' );
