...
if ( foxiz_get_option( 'single_post_comment_button' ) ) {
...
?><div id="rb-user-reviews-<?php echo esc_attr( get_the_ID() ); ?>" class="comment-box-wrap entry-sec rb-user-reviews">
    <div class="comment-box-header">
	    <?php if ( foxiz_get_option( 'single_post_comment_button' ) || foxiz_is_amp() ) :
		    $review_link = '#';
		    if ( foxiz_is_amp() ) {
			    $review_link =  get_comments_link( $post );
			    $review_link = add_query_arg( 'noamp', 'mobile', $review_link );
		    }
		    ?><span class="comment-box-title h3"><?php foxiz_render_svg( 'feedback' ); ?><span class="is-invisible"><?php echo foxiz_get_review_heading( get_the_ID() ); ?></span></span>
            <a href="<?php echo esc_url($review_link); ?>" class="show-post-comment"><?php foxiz_render_svg( 'feedback' ); ?><?php echo foxiz_get_review_heading( get_the_ID() ); ?></a>
		<?php else: ?>
            <span class="h3"><?php foxiz_render_svg( 'feedback' ); ?><?php echo foxiz_get_review_heading( get_the_ID() ); ?></span>
		<?php endif; ?>
    </div>
	<?php if ( ! foxiz_is_amp() ) : ?>
			    ...
			    <div id="comments" class="comments-area rb-reviews-area">
				    <?php if ( have_comments() ) : ?>
					    <div class="rb-review-list rb-section">
						    <ul class="comment-list entry"><?php wp_list_comments( array( 'callback' => 'foxiz_user_review_list' ) ); ?></ul>
